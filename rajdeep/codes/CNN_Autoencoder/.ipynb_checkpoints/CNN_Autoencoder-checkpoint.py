...
class CNNAutoencoder(nn.Module):
    ...
    def process_batch(self, batch, train=False):
        ...
        if train:
            batch_loss.backward()
            self.optimizer.step()
        return hidden_representation, batch_loss.item()
    ...
    def evaluate_model(self, dataLoader):
        ...
        for batch in dataLoader:
            hidden_representation, batch_loss = self.process_batch(batch, train=False)
    ...
    def train_model(self, trainDataLoader, devDataLoader):
        ...
        for epoch in range(self.epochs):
            ...
            for idx, batch in tqdm(enumerate(trainDataLoader)):
                _, batch_loss = self.process_batch(batch, train=True)
