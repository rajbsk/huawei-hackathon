...
def read_folder_files_regression(folder_name, steps):
	...
	for file in files:		
		...
		inversed_values = [1-value for value in normalized_kpi_values]
		data += split_sequence_regression(inversed_values, steps)
	return data
...
def read_folder_files_prediction(folder_name, steps):
	...
	for file in files:
		if "csv" in file:
			...
			inversed_values = [1-value for value in normalized_kpi_values]
			data += split_sequence_prediction(inversed_values, labels, steps)
